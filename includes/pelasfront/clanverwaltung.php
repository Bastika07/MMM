...
if ($nLoginID == "") {
	...
	$row = DB::query("select c.NAME, uc.CLANID, uc.AUFNAHMESTATUS from USER_CLAN uc, CLAN c where c.MANDANTID = $nPartyID and c.CLANID = uc.CLANID and uc.USERID = ".intval($nLoginID)." and uc.MANDANTID = $nPartyID")->fetch_assoc();
	$inClan = $row['CLANID'];
	$Status = $row['AUFNAHMESTATUS'];
	$derClan= $row['NAME'];
	//eingeloggter User bezahlt?
	$row = DB::query("select STATUS, RABATTSTUFE, BEZ_IN_CLAN from ASTATUS where MANDANTID = $nPartyID and USERID = ".intval($nLoginID)."")->fetch_assoc();
	...
	$row = DB::query("select LOGIN from USER where USERID = '".intval($_GET['nClanmate'])."'")->fetch_assoc();
	$sClanmateName = db2display($row['LOGIN']);
	// Clan von $nClanmate rausfinden
	$row = DB::query("select uc.CLANID from USER_CLAN uc, CLAN c where c.MANDANTID = $nPartyID and c.CLANID = uc.CLANID and uc.USERID = '".intval($_GET['nClanmate'])."' and uc.MANDANTID = $nPartyID")->fetch_assoc();
	...
	if ($_POST['actionUpload'] == "upload") {
		...
		if ($_FILES['iUserbild']['name'] != "") {
			if ($_FILES['iUserbild']['size'] > 20000) {
				...
				$row = DB::query("select CLANID from USER_CLAN where MANDANTID = '$nPartyID' and USERID = '".intval($nLoginID)."'")->fetch_assoc();
	...
	if ($_GET['action'] == "entf") {
		if ($nClanmatesClan != $inClan) {
			...
			$row = DB::query("delete from USER_CLAN where MANDANTID = '$nPartyID' and USERID = '".intval($_GET['nClanmate'])."'")->fetch_assoc();
		...
		/*$result = DB::query("select u.LOGIN, uc.CLANID from USER_CLAN uc, CLAN c, USER u where u.USERID = uc.USERID and c.MANDANTID = $nPartyID and c.CLANID = uc.CLANID and uc.USERID = $nClanmate and uc.MANDANTID = $nPartyID");
		...
		if ($inClan != $row[CLANID]) {
			...
			if ($check != "yes") {
				...
				DB::query("update ASTATUS set STATUS = '$userStatus', RABATTSTUFE = '$userRabatt', BEZ_IN_CLAN = '$userBezClan' where MANDANTID = $nPartyID and USERID = '$nClanmate'");
				//echo DB::$link->errno.": ".DB::$link->error."<BR>";
				DB::query("update ASTATUS set STATUS = $STATUS_ANGEMELDET, RABATTSTUFE = 0, BEZ_IN_CLAN = 0, WERGEAENDERT = $nLoginID where MANDANTID = $nPartyID and USERID = $nLoginID");
				echo "<p>Du hast Deine &Uuml;berweisung und Sitzplatz dem User &quot;".db2display($row[LOGIN])."&quot; zugeordnet.</p>";
				
				//Sitzplatz? Welche Ebene?
				$result = DB::query("select d.EBENE from SITZ s, SITZDEF d where d.MANDANTID = $nPartyID and d.REIHE=s.REIHE and s.USERID = $nLoginID and s.MANDANTID = $nPartyID");
				$row = $result->fetch_array();
				$ebene = $row[EBENE];
				
				// Neuen User auf alten Seat
				DB::query("update SITZ set USERID = $nClanmate where MANDANTID = $nPartyID and USERID = $nLoginID");
		...
		$result = DB::query("select STRINGWERT from CONFIG where MANDANTID = $nPartyID and PARAMETER='ANMELDUNG_OFFEN'");
		$row = $result->fetch_array();
		if ($row['STRINGWERT'] != "J") {
			echo "<p>Die Anmeldung zu dieser Party wurde noch nicht er&ouml;ffnet.</p>";
		} else {
			//Checken, ob auch in gleichem Clan!
			$result = DB::query("select uc.CLANID from USER_CLAN uc, CLAN c where c.MANDANTID = $nPartyID and c.CLANID = uc.CLANID and uc.USERID = '".intval($nClanmate)."' and uc.MANDANTID = $nPartyID");
			$row = $result->fetch_array();

			if ($inClan != $row[CLANID]) {
				echo "<p>Fehler: Dieses Clanmitglied ist nicht in Deinem Clan.</p>";
			} else {
				$result = DB::query("select USERID from ASTATUS where USERID=".intval($nClanmate)." and MANDANTID=$nPartyID");
				$row = $result->fetch_array();
				if ($row['USERID'] > 0 ) {
					DB::query("update ASTATUS set STATUS = $STATUS_ANGEMELDET, WERGEAENDERT='".intval($nLoginID)."', WANNANGEMELDET=NOW() where USERID='".intval($nClanmate)."' and MANDANTID=$nPartyID");
				} else {
					DB::query("insert into ASTATUS (MANDANTID, USERID, STATUS, WERGEAENDERT, WANNANGEMELDET) values ($nPartyID, '".intval($nClanmate)."', $STATUS_ANGEMELDET, '".intval($nLoginID)."', NOW())");
				}
		...
		$row = DB::query("select CLANID, NAME, URL, FANPROGRAMM from CLAN where NAME = '".safe($_POST['iClanName'])."' and MANDANTID = $nPartyID and CLANID <> '".intval($inClan)."'")->fetch_assoc();
		$exists = 0;
		if ($row['CLANID'] > 0) {
			$exists = 1;
		}
		...
		if (!isset($_POST['go']) ) {
			...
			if ($_GET['action'] == "edit") {
				//update!
				$result = DB::query("update CLAN set NAME = '".safe($_POST['iClanName'])."', URL = '".safe($_POST['iURL'])."', IRC_CHANNEL = '".safe($_POST['iIRC_CHANNEL'])."', FANPROGRAMM='".safe($_POST['iFanprogramm'])."', WERGEAENDERT = ".intval($nLoginID)." where MANDANTID = $nPartyID and CLANID = ".intval($inClan) );
				//echo DB::$link->errno.": ".DB::$link->error."<BR>";
				...
				//Clan erstellen
				$result = DB::query("insert into CLAN (MANDANTID, NAME, URL, IRC_CHANNEL, FANPROGRAMM, WANNANGELEGT, WERANGELEGT, WERGEAENDERT) values ($nPartyID, '".safe($_POST['iClanName'])."', '".safe($_POST['iURL'])."', '".safe($_POST['iIRC_CHANNEL'])."', '".safe($_POST['iFanprogramm'])."', NOW(), ".intval($nLoginID).", ".intval($nLoginID).")");
				//ID des grad erstellten Clans feststellen
				$row = DB::query("select CLANID from CLAN where NAME = '".safe($_POST['iClanName'])."' and MANDANTID = $nPartyID")->fetch_assoc();
				$newClanID = $row['CLANID'];
				//Ersteller hinzufuegen
				$result = DB::query("insert into USER_CLAN (MANDANTID, USERID, CLANID, AUFNAHMESTATUS, WANNANGELEGT, WERANGELEGT, WERGEAENDERT) values ('$nPartyID', ".intval($nLoginID).", ".intval($newClanID).", $AUFNAHMESTATUS_OK, NOW(), ".intval($nLoginID).", ".intval($nLoginID).")");
				if (DB::$link->errno == 0) {
		...
		if ($_GET['clan'] < 1) {
			...
			$result = DB::query("select CLANID, NAME, URL from CLAN where MANDANTID = $nPartyID order by NAME");
		...
		if ($inClan == "") {
			...
			if ($Status == $AUFNAHMESTATUS_WARTEND) {
				if ($_GET['action'] == "revoke") {
					$row = DB::query(" delete from USER_CLAN where AUFNAHMESTATUS = $AUFNAHMESTATUS_WARTEND and MANDANTID = $nPartyID and USERID = ".intval($nLoginID))->fetch_assoc();
				...
				if ($_GET['action'] == "finaljoin") {
					$row = DB::query("update USER_CLAN set AUFNAHMESTATUS = $AUFNAHMESTATUS_OK where MANDANTID = $nPartyID and USERID = '".intval($_GET['member'])."'")->fetch_assoc();
					echo "<p>Du hast soeben ein neues Mitglied aufgenommen.</p>";
					echo "<p><img src=\"gfx/headline_pfeil.png\" border=\"0\"> <a href=\"?page=18\">Zur Clanverwaltung</a></p>";
				} elseif ($_GET['action'] == "off") {
					//austreten nur, wenn nicht bezahlt!
					$row = DB::query("select STATUS from ASTATUS where MANDANTID = $nPartyID and USERID = $nLoginID and RABATTSTUFE > 0")->fetch_assoc();
					//echo DB::$link->errno.": ".DB::$link->error."<BR>";
					...
					if ($row['STATUS'] == $STATUS_BEZAHLT || $row['STATUS'] == $STATUS_BEZAHLT_LOGE) {
						...
						$row = DB::query("delete from USER_CLAN where AUFNAHMESTATUS = $AUFNAHMESTATUS_OK and MANDANTID = $nPartyID and USERID = $nLoginID and CLANID='".intval($_GET['clan'])."'")->fetch_assoc();
						//wenn letztes Mitglied, alle Relikte loeschen
						$result = DB::query("select count(*) from USER_CLAN where AUFNAHMESTATUS = $AUFNAHMESTATUS_OK and MANDANTID = $nPartyID and CLANID='".intval($_GET['clan'])."'");
						$row_count = $result->fetch_array();
						if ($row_count[0] == 0 || $row_count[0] == "") {
							//delete clan completely!
							$row = DB::query("delete from USER_CLAN where AUFNAHMESTATUS = $AUFNAHMESTATUS_WARTEND and MANDANTID = $nPartyID and CLANID='".intval($_GET['clan'])."'")->fetch_assoc();
							$row = DB::query("delete from CLAN where MANDANTID = $nPartyID and CLANID='".intval($_GET['clan'])."'")->fetch_assoc();
						}
					...
					$row = DB::query("select * from CLAN where CLANID = '".intval($inClan)."'")->fetch_assoc();
					...
					$result = DB::query("select u.LOGIN, u.USERID from USER u, USER_CLAN uc where uc.AUFNAHMESTATUS = $AUFNAHMESTATUS_OK and u.USERID = uc.USERID and uc.CLANID = '".intval($inClan)."' and uc.MANDANTID = $nPartyID");
					...
					while ($row=$result->fetch_array()) {
						...
						if (ACCOUNTING == "OLD") {
							...
							$result2 = DB::query("select a.STATUS from ASTATUS a where a.USERID = $row[USERID] and a.MANDANTID = $nPartyID");
							$row_status = $result2->fetch_array();
					...
					$result = DB::query("select u.LOGIN, u.USERID from USER u, USER_CLAN uc where uc.AUFNAHMESTATUS = $AUFNAHMESTATUS_WARTEND and u.USERID = uc.USERID and uc.CLANID = '".intval($inClan)."' and uc.MANDANTID = $nPartyID");
					while ($row=$result->fetch_array()) {
					...
					echo "<hr><p><b>Clanrabatte</b></p>";
						//Wie viele Mebers?					
						$result = DB::query("select count(*) from USER_CLAN where AUFNAHMESTATUS = $AUFNAHMESTATUS_OK and MANDANTID = $nPartyID and CLANID='".intval($inClan)."'");
						$row_count = $result->fetch_array();
						$anzahl= $row_count[0];
						...
						if ($Rabatt5 <= 0 && $Rabatt10 <=0 && $Rabatt15 <=0) {
							...
							if ($anzahl >= 5) {
								...
								for ($i=1;$i<=$anzahl;$i++) {
									...
									$row = DB::query("select STRINGWERT from CONFIG where PARAMETER = '$rabatt' and MANDANTID = $nPartyID")->fetch_assoc();
								...
								$row = DB::query("select STRINGWERT from CONFIG where PARAMETER = 'KONTO_NAME' and MANDANTID = $nPartyID")->fetch_assoc(); $config[KONTO_NAME] = $row[STRINGWERT];
								$row = DB::query("select STRINGWERT from CONFIG where PARAMETER = 'KONTO_NUMMER' and MANDANTID = $nPartyID")->fetch_assoc(); $config[KONTO_NUMMER] = $row[STRINGWERT];
								$row = DB::query("select STRINGWERT from CONFIG where PARAMETER = 'KONTO_BLZ' and MANDANTID = $nPartyID")->fetch_assoc(); $config[KONTO_BLZ] = $row[STRINGWERT];
								$row = DB::query("select STRINGWERT from CONFIG where PARAMETER = 'KONTO_BANK' and MANDANTID = $nPartyID")->fetch_assoc(); $config[KONTO_BANK] = $row[STRINGWERT];
								$row = DB::query("select BESCHREIBUNG, EMAIL from MANDANT where MANDANTID=$nPartyID")->fetch_assoc(); $config[MANDANTNAME] = $row[BESCHREIBUNG];
