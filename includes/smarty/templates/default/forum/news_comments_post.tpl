...
<tr height="180">
  ...
  <td valign="top" class="hblau" {if $hidden}style="background-color: #a0a0a0"{/if}>
    <table width="99%">
      <tr>
        <td align="left">{$time|date_format:"%d.%m.%Y %H:%M"} {if $val.isNew}<i>new</i>{/if}</td>
        <td align="right">
        <a href="?page=2&action=addComment&newsID={$thread->id}&postToQuoteId={$contentID}" class="forumlink"><img src="gfx/forum_quote.gif" border="0"></a>
        {if $edit}
          <a href="?page=2&action=edit&post={$contentID}"><img src="gfx/forum_edit.gif" border="0"></a>
        {/if}
        {if $admin}
          {if $hidden}
            <a href="?page=2&action=changemode&mode=hidden&post={$contentID}">anzeigen</a>
          {else}
            <a href="?page=2&action=changemode&mode=hidden&post={$contentID}">verstecken</a>
          {/if}
        {/if}
        </td>
      </tr>
