...
function shorten_url($url, $maxlength = 40) {
  if (strlen($url) - DOTS_LEN > $maxlength) {
    $retval = substr($url, 0, $maxlength/2);
    $retval .= DOTS;
    $retval .= substr($url, strlen($url)-$maxlength/2);
  } else {
    $retval = $url;
  }
  return $retval;
...
function smarty_modifier_bbcode2html($message) {
  $preg = array(
    ...
    // [url]
    '/\[url(?::\w+)?\]www\.(.*?)\[\/url(?::\w+)?\]/sie'        => "'<a href=\"http://www.\\1\" target=\"_blank\" class=\"ng_url\">'.shorten_url('www.'.'\\1').'</a>'",
    '/\[url(?::\w+)?\](.*?)\[\/url(?::\w+)?\]/sie'             => "'<a href=\"\\1\" target=\"_blank\" class=\"ng_url\">'.shorten_url('\\1').'</a>'",
    '/\[url=(.*?)(?::\w+)?\](.*?)\[\/url(?::\w+)?\]/si'       => "<a href=\"\\1\" target=\"_blank\" class=\"ng_url\">\\2</a>",
    '/\[urlintern(?::\w+)?\]www\.(.*?)\[\/urlintern(?::\w+)?\]/si'        => "<a href=\"http://www.\\1\" class=\"ng_url\">\\1</a>",
    '/\[urlintern(?::\w+)?\](.*?)\[\/urlintern(?::\w+)?\]/si'             => "<a href=\"\\1\" class=\"ng_url\">\\1</a>",
    '/\[urlintern=(.*?)(?::\w+)?\](.*?)\[\/urlintern(?::\w+)?\]/si'       => "<a href=\"\\1\" class=\"ng_url\">\\2</a>",
    // [img]
    '/\[img(?::\w+)?\](.*?)\[\/img(?::\w+)?\]/si'             => "<img src=\"\\1\" border=\"0\" vspace=3 hspace=3 />",
    '/\[imgcenter(?::\w+)?\](.*?)\[\/imgcenter(?::\w+)?\]/si'             => "<div align=\"center\"><img src=\"\\1\" border=\"0\" vspace=3 hspace=3/></div>",
    '/\[imgleft(?::\w+)?\](.*?)\[\/imgleft(?::\w+)?\]/si'     => "<img align=\"left\" src=\"\\1\" border=\"0\" vspace=3 style=\"margin-right: 15px\" />",
    '/\[imgright(?::\w+)?\](.*?)\[\/imgright(?::\w+)?\]/si'   => "<img align=\"right\" src=\"\\1\" border=\"0\" vspace=3 style=\"margin-left: 15px\" />",
    '/\[img=(.*?)x(.*?)(?::\w+)?\](.*?)\[\/img(?::\w+)?\]/si' => "<img width=\"\\1\" height=\"\\2\" src=\"\\3\" border=\"0\" vspace=3 hspace=3 />",
    // [quote]
    '/\[quote(?::\w+)?\](.*?)\[\/quote(?::\w+)?\]/si'         => "<div class=\"ng_quote_head\">Quote:</div><div class=\"ng_quote_body\">\\1</div>",
    '/\[quote=(?:&quot;|"|\')(.*?)["\']?(?:&quot;|"|\')\](.*?)\[\/quote(?::\w+)?\]/si'   => "<div class=\"ng_quote_head\">Quote \\1:</div><div class=\"ng_quote_body\">\\2</div>",    
    // [list]
  ...
  $preg_keys = array_keys($preg);
  $preg_values = array_values($preg);


  // Inhalt aus [code][/code] sichern
  $codeContent = array();  
  
  for ($i = 0; preg_match('/\[code(?::\w+)?\](.*?)\[\/code(?::\w+)?\]/si', $message, $matches); $i++) {
    $message = preg_replace('/\[code(?::\w+)?\](.*?)\[\/code(?::\w+)?\]/si', "@@SAVEDCODE-$i@@", $message, 1);
    $codeContent[$i] = $matches[1];        
  }  
  $codeTags = $i;
  
  $bbcode_left = true;
  while ($bbcode_left) {
    $message = preg_replace($preg_keys, $preg_values, $message);
    $bbcode_left = false;
    foreach ($preg_keys as $val) {      
      if (preg_match($val, $message) != 0) {
        $bbcode_left = true;
        break;
      }
    }
  ...
  // Inhalt von [code][/code] wieder einfügen
  for ($i = 0; $i < $codeTags; $i++) {
    $message = preg_replace("/@@SAVEDCODE-$i@@/si", "<div class=\"ng_code\">{$codeContent[$i]}</div>", $message, 1);
  }
