...
<?php
	...
	# Mandant
	$res = mysql_db_query ($dbname, "select * from MANDANT where MANDANTID='$mandant'", $dbh);
	...
	# Teamdaten
	$res = mysql_db_query($dbname, "SELECT * FROM USER_EXT");
	fwrite($fhandle, "delete from USER_EXT;\n");
	while ($row = mysql_fetch_assoc($res))
		fwrite($fhandle, "INSERT INTO USER_EXT SET ".unroll_arr($row).";\n");

	# Rechte
	$res = mysql_db_query($dbname, "SELECT * FROM RECHTZUORDNUNG where MANDANTID=$mandant");
	fwrite($fhandle, "delete from RECHTZUORDNUNG;\n");
	while ($row = mysql_fetch_assoc($res))
		fwrite($fhandle, "INSERT INTO RECHTZUORDNUNG SET ".unroll_arr($row).";\n");

	# Anmeldestatus
	$result = mysql_db_query($dbname, "select * from ASTATUS where MANDANTID=$mandant", $dbh);
	...
	# Config
	$result = mysql_db_query($dbname, "select PARAMETER, MANDANTID, STRINGWERT, BESCHREIBUNG from CONFIG where MANDANTID=$mandant", $dbh);
	...
	# Sitzplan
	$result = mysql_db_query($dbname, "select MANDANTID, EBENE , XCORD , YCORD ,REIHE ,LAENGE ,AUSRICHTUNG ,ISTLOGE, ISTGESPERRT  from SITZDEF where MANDANTID=$mandant", $dbh);
	...
	# Sitzplatz
	$result = mysql_db_query($dbname, "select MANDANTID, USERID, REIHE, PLATZ, RESTYP from SITZ where MANDANTID=$mandant", $dbh);
	...
	# Sitzgruppen
	$result = mysql_db_query($dbname, "select * from sitzgruppe where MANDANTID='$mandant'", $dbh);
	...
	# nochma Sitzplan
	$result = mysql_db_query($dbname, "select * from sitzplan_def where mandantID='$mandant'", $dbh);
	...
	$result = mysql_db_query($dbname, "select * from sitzgruppen_mitglieder where mandantID='$mandant'", $dbh);
	...
	# Clan blub
	$result = mysql_db_query($dbname, "select MANDANTID, CLANID, NAME, URL, IRC_CHANNEL, WANNANGELEGT, WERANGELEGT, WANNGEAENDERT, WERGEAENDERT from CLAN where MANDANTID=$mandant", $dbh);
	...
	# Zuordnung User-Clan
	$result = mysql_db_query($dbname, "select MANDANTID, USERID, CLANID, AUFNAHMESTATUS, WANNANGELEGT, WERANGELEGT, WANNGEAENDERT, WERGEAENDERT from USER_CLAN where MANDANTID=$mandant", $dbh);
	...
	# Gastserver
	$result = mysql_db_query($dbname, "select * from GASTSERVER where MANDANTID=$mandant", $dbh);
	...
	$res = mysql_db_query($dbname, "SELECT * FROM t_turnier WHERE partyid = '{$partyid}'");
	while ($row = mysql_fetch_assoc($res)) {
	    fwrite($fhandle, "INSERT INTO t_turnier SET ".unroll_arr($row).";\n");

	    $res2 = mysql_db_query($dbname, "SELECT * FROM t_admin WHERE turnierid = '{$row['turnierid']}'");
	    while ($row2 = mysql_fetch_assoc($res2)) {
		fwrite($fhandle, "INSERT INTO t_admin SET ".unroll_arr($row2).";\n");
	    }

	    $res2 = mysql_db_query($dbname, "SELECT * FROM t_preise WHERE turnierid = '{$row['turnierid']}'");
	    while ($row2 = mysql_fetch_assoc($res2)) {
		fwrite($fhandle, "INSERT INTO t_preise SET ".unroll_arr($row2).";\n");
	    }

	    $res2 = mysql_db_query($dbname, "SELECT * FROM t_team WHERE turnierid = '{$row['turnierid']}'");
	    while ($row2 = mysql_fetch_assoc($res2)) {
		fwrite($fhandle, "INSERT INTO t_team SET ".unroll_arr($row2).";\n");

		$res3 = mysql_db_query($dbname, "SELECT * FROM t_team2user WHERE turnierid = '{$row['turnierid']}' AND teamid = '{$row2['teamid']}'");
	...
	$res = mysql_db_query($dbname, "SELECT * FROM party where mandantId='{$partyid}'");
	fwrite($fhandle, "DELETE FROM party;\n");
	while ($row = mysql_fetch_assoc($res)) {
	    fwrite($fhandle, "INSERT INTO party SET ".unroll_arr($row).";\n");
	}

 	$res = mysql_db_query($dbname, "SELECT t.* FROM acc_ticket_typ t, party p where p.mandantId='{$partyid}' and p.partyId=t.partyId and p.aktiv='J'");
	fwrite($fhandle, "DELETE FROM acc_ticket_typ;\n");
 	while ($row = mysql_fetch_assoc($res)) {
 	    fwrite($fhandle, "INSERT INTO acc_ticket_typ SET ".unroll_arr($row).";\n");
	}

	$res = mysql_db_query($dbname, "SELECT b.* FROM acc_bestellung b, party p where p.mandantId='{$partyid}' and p.partyId=b.partyId and p.aktiv='J'");
	fwrite($fhandle, "DELETE FROM acc_bestellung;\n");
	while ($row = mysql_fetch_assoc($res)) {
	    fwrite($fhandle, "INSERT INTO acc_bestellung SET ".unroll_arr($row).";\n");
	}

	$res = mysql_db_query($dbname, "SELECT t.* FROM acc_tickets t, party p where p.mandantId='{$partyid}' and p.partyId=t.partyId and p.aktiv='J'");
	fwrite($fhandle, "DELETE FROM acc_tickets;\n");
	while ($row = mysql_fetch_assoc($res)) {
	    fwrite($fhandle, "INSERT INTO acc_tickets SET ".unroll_arr($row).";\n");
	}

	$res = mysql_db_query($dbname, "SELECT a.* FROM aussteller a, party p where p.mandantId='{$partyid}' and p.partyId=a.partyId and p.aktiv='J'");
