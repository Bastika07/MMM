...
$smarty->appName = 'sliderbilder';

if (!is_dir(SLIDER_DIR))
  die(SLIDER_DIR.' fehlt!');
...
while ($row = $res->fetch_assoc()) {
  ...
  $pattern = SLIDER_DIR.$row['MANDANTID'].'_*';
...
switch($action) {
  ...
  case 'preview':
    ...
    } else if (!file_exists(SLIDER_DIR.$imageName)) {
      $error = 'Datei existiert nicht';    
    } else {
      $image['name'] = $imageName;
      $image['info'] = getimagesize(SLIDER_DIR.$imageName);
      $image['groesse'] = filesize(SLIDER_DIR.$imageName);
      $image['url'] = SLIDER_PATH.$imageName;
      $smarty->assign('image', $image);
    }
    if (isset($error)) 
  ...
  case 'upload':
    ...
    $dstFilename = SLIDER_DIR.$_POST['mandantId'].'_'.$_FILES['uploadImage']['name'];
    ...
    if (!isset($_POST['mandantId']) || !is_numeric($_POST['mandantId']) || !isset($mandanten[$_POST['mandantId']])) {
      ...
      $val = handleUpload(SLIDER_DIR, $_POST['mandantId'].'_');
      if ($val !== true) {
        $error = $val;
      } else {
        ...
        $smarty->assign('uploadedImageUrl', SLIDER_PATH.$dstFilenameWithoutDir);
      }      
    }
    if (isset($error))
      $smarty->assign('error', $error);
  ...
  case 'delete':
    ...
    if (!isset($_GET['confirm']) || !$_GET['confirm']) {
      ...
      if (($mandantId = validateImageName($imageName)) === false) {
        ...
        $error = 'Datei existiert nicht.';    
      } else if (!unlink(SLIDER_DIR.$imageName)) {
...
function validateImageName($imageName) {
   ...
   */
  if (preg_match(SLIDER_VALID_FILE_PATTERN, $imageName, $m) == 1)
    $rc = (int) $m[1];
  else
    $rc = (bool) false;
  return $rc;
...
function validateImageType($imageType) {
   ...
   */
  global $SLIDER_VALID_FILE_EXTS;
  return in_array($imageType, $SLIDER_VALID_FILE_EXTS);
}
