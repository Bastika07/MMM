...
$smarty->appName = 'locationbilder';

if (!is_dir(LOCATION_DIR))
  die(LOCATION_DIR.' fehlt!');
...
while ($row = $res->fetch_assoc()) {
  ...
  $pattern = LOCATION_DIR.$row['MANDANTID'].'_*';
...
switch($action) {
  ...
  case 'preview':
    ...
    } else if (!file_exists(LOCATION_DIR.$imageName)) {
      $error = 'Datei existiert nicht';    
    } else {
      $image['name'] = $imageName;
      $image['info'] = getimagesize(LOCATION_DIR.$imageName);
      $image['groesse'] = filesize(LOCATION_DIR.$imageName);
      $image['url'] = LOCATION_PATH.$imageName;
      $smarty->assign('image', $image);
    }
    if (isset($error)) 
  ...
  case 'upload':
    ...
    $dstFilename = LOCATION_DIR.$_POST['mandantId'].'_'.$_FILES['uploadImage']['name'];
    ...
    if (!isset($_POST['mandantId']) || !is_numeric($_POST['mandantId']) || !isset($mandanten[$_POST['mandantId']])) {
      ...
      $val = handleUpload(LOCATION_DIR, $_POST['mandantId'].'_');
      if ($val !== true) {
        $error = $val;
      } else {
        ...
        $smarty->assign('uploadedImageUrl', LOCATION_PATH.$dstFilenameWithoutDir);
      }      
    }
    if (isset($error))
      $smarty->assign('error', $error);
  ...
  case 'delete':
    ...
    if (!isset($_GET['confirm']) || !$_GET['confirm']) {
      ...
      if (($mandantId = validateImageName($imageName)) === false) {
        ...
        $error = 'Datei existiert nicht.';    
      } else if (!unlink(LOCATION_DIR.$imageName)) {
...
function validateImageName($imageName) {
   ...
   */
  if (preg_match(LOCATION_VALID_FILE_PATTERN, $imageName, $m) == 1)
    $rc = (int) $m[1];
  else
    $rc = (bool) false;
  return $rc;
...
function validateImageType($imageType) {
   ...
   */
  global $LOCATION_VALID_FILE_EXTS;
  return in_array($imageType, $LOCATION_VALID_FILE_EXTS);
}
