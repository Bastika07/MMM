...
$smarty->appName = 'newsbild';

if (!is_dir(NEWSBILD_DIR))
  die(NEWSBILD_DIR.' fehlt!');
...
while ($row = $res->fetch_assoc()) {
  ...
  $pattern = NEWSBILD_DIR.$row['MANDANTID'].'_*';
...
switch($action) {
  ...
  case 'preview':
    ...
    } else if (!file_exists(NEWSBILD_DIR.$imageName)) {
      $error = 'Datei existiert nicht';    
    } else {
      $image['name'] = $imageName;
      $image['info'] = getimagesize(NEWSBILD_DIR.$imageName);
      $image['groesse'] = filesize(NEWSBILD_DIR.$imageName);
      $image['url'] = NEWSBILD_PATH.$imageName;
      $smarty->assign('image', $image);
    }
    if (isset($error)) 
  ...
  case 'upload':
    ...
    $dstFilename = NEWSBILD_DIR.$_POST['mandantId'].'_'.$_FILES['uploadImage']['name'];
    ...
    if (!isset($_POST['mandantId']) || !is_numeric($_POST['mandantId']) || !isset($mandanten[$_POST['mandantId']])) {
      ...
      $val = handleUpload(NEWSBILD_DIR, $_POST['mandantId'].'_');
      if ($val !== true) {
        $error = $val;
      } else {
        ...
        $smarty->assign('uploadedImageUrl', NEWSBILD_PATH.$dstFilenameWithoutDir);
      }      
    }
    if (isset($error))
      $smarty->assign('error', $error);
  ...
  case 'delete':
    ...
    if (!isset($_GET['confirm']) || !$_GET['confirm']) {
      ...
      if (($mandantId = validateImageName($imageName)) === false) {
        ...
        $error = 'Datei existiert nicht.';    
      } else if (!unlink(NEWSBILD_DIR.$imageName)) {
...
function validateImageName($imageName) {
   ...
   */
  if (preg_match(NEWSBILD_VALID_FILE_PATTERN, $imageName, $m) == 1)
    $rc = (int) $m[1];
  else
    $rc = (bool) false;
  return $rc;
...
function validateImageType($imageType) {
   ...
   */
  global $NEWSBILD_VALID_FILE_EXTS;
  return in_array($imageType, $NEWSBILD_VALID_FILE_EXTS);
}
