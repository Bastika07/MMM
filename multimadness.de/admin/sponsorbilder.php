...
$smarty->appName = 'sponsorbilder';

if (!is_dir(SPONSOR_DIR))
  die(SPONSOR_DIR.' fehlt!');
...
while ($row = mysql_fetch_assoc($res)) {
  ...
  $pattern = SPONSOR_DIR.$row['MANDANTID'].'_*';
...
switch($action) {
  ...
  case 'preview':
    ...
    } else if (!file_exists(SPONSOR_DIR.$imageName)) {
      $error = 'Datei existiert nicht';    
    } else {
      $image['name'] = $imageName;
      $image['info'] = getimagesize(SPONSOR_DIR.$imageName);
      $image['groesse'] = filesize(SPONSOR_DIR.$imageName);
      $image['url'] = SPONSOR_PATH.$imageName;
      $smarty->assign('image', $image);
    }
    if (isset($error)) 
  ...
  case 'upload':
    ...
    $dstFilename = SPONSOR_DIR.$_POST['mandantId'].'_'.$_FILES['uploadImage']['name'];
    ...
    if (!isset($_POST['mandantId']) || !is_numeric($_POST['mandantId']) || !isset($mandanten[$_POST['mandantId']])) {
      ...
      $val = handleUpload(SPONSOR_DIR, $_POST['mandantId'].'_');
      if ($val !== true) {
        $error = $val;
      } else {
        ...
        $smarty->assign('uploadedImageUrl', SPONSOR_PATH.$dstFilenameWithoutDir);
      }      
    }
    if (isset($error))
      $smarty->assign('error', $error);
  ...
  case 'delete':
    ...
    if (!isset($_GET['confirm']) || !$_GET['confirm']) {
      ...
      if (($mandantId = validateImageName($imageName)) === false) {
        ...
        $error = 'Datei existiert nicht.';    
      } else if (!unlink(SPONSOR_DIR.$imageName)) {
...
function validateImageName($imageName) {
   ...
   */
  if (preg_match(SPONSOR_VALID_FILE_PATTERN, $imageName, $m) == 1)
    $rc = (int) $m[1];
  else
    $rc = (bool) false;
  return $rc;
...
function validateImageType($imageType) {
   ...
   */
  global $SPONSOR_VALID_FILE_EXTS;
  return in_array($imageType, $SPONSOR_VALID_FILE_EXTS);
}
