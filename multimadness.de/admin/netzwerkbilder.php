...
$smarty->appName = 'netzwerkbilder';

if (!is_dir(NETZWERK_DIR))
  die(NETZWERK_DIR.' fehlt!');
...
switch($action) {
  ...
  case 'preview':
    ...
    } else if (!file_exists(NETZWERK_DIR.$imageName)) {
      $error = 'Datei existiert nicht';    
    } else {
      $image['name'] = $imageName;
      $image['info'] = getimagesize(NETZWERK_DIR.$imageName);
      $image['groesse'] = filesize(NETZWERK_DIR.$imageName);
      $image['url'] = NETZWERK_PATH.$imageName;
      $smarty->assign('image', $image);
    }
    if (isset($error)) 
  ...
  case 'upload':
    ...
    $dstFilename = NETZWERK_DIR.$_POST['mandantId'].'_'.$_FILES['uploadImage']['name'];
    ...
    if (!isset($_POST['mandantId']) || !is_numeric($_POST['mandantId']) || !isset($mandanten[$_POST['mandantId']])) {
      ...
      $val = handleUpload(NETZWERK_DIR, $_POST['mandantId'].'_');
      if ($val !== true) {
        $error = $val;
      } else {
        ...
        $smarty->assign('uploadedImageUrl', NETZWERK_PATH.$dstFilenameWithoutDir);
      }      
    }
    if (isset($error))
      $smarty->assign('error', $error);
  ...
  case 'delete':
    ...
    if (!isset($_GET['confirm']) || !$_GET['confirm']) {
      ...
      if (($mandantId = validateImageName($imageName)) === false) {
        ...
        $error = 'Datei existiert nicht.';    
      } else if (!unlink(NETZWERK_DIR.$imageName)) {
...
function validateImageName($imageName) {
   ...
   */
  if (preg_match(NETZWERK_VALID_FILE_PATTERN, $imageName, $m) == 1)
    $rc = (int) $m[1];
  else
    $rc = (bool) false;
  return $rc;
...
function validateImageType($imageType) {
   ...
   */
  global $NETZWERK_VALID_FILE_EXTS;
  return in_array($imageType, $NETZWERK_VALID_FILE_EXTS);
}
